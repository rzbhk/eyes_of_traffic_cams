...
def get_prepare_data(df, BATCH_SIZE, TIMESTEPS):
    def prepare_data(mode="train"):
        if mode == "train":
            while True:
                for b in range(0, L - TIMESTEPS, BATCH_SIZE):
                    lst_in = []
                    lst_out = []
                    for i in range(b, min(b + BATCH_SIZE, L - TIMESTEPS)):
                        lst_in.append(df.iloc[i:i + TIMESTEPS, :].values)
                        lst_out.append(df.iloc[i + TIMESTEPS, :].values)

                    yield np.stack(lst_in),np.stack(lst_out)
...
def get_model(params):
    matrix_in = Input(shape=(params["timesteps"], CAMERA_FEATURES), )
...
if __name__ == "__main__":
    ...
    for params in paramss:
        ...
        prepare_data = get_prepare_data(df, params["batch_size"], params["timesteps"]) #partial function
